...
<Illustration
    description="Each Actor run has a status (the `status` field), which indicates its stage in the Actor's lifecycle."
    position="right"
    image={illuAPIStatus}
    noCaption
/>
...
<Illustration
    description="Abort itself or another Actor running on the Apify platform. Aborting an Actor changes its status to ABORTED."
    position="right"
    image={illuAPIAbort}
    noCaption
/>
