...
<Illustration description="Actors are designed to be easy to develop, share, integrate, and build upon." />
...
<Illustration description="Each Actor accepts an input object, which tells it what it should do." position="content" />
...
<Illustration description="The Actors run within an isolated Docker container" position="right" />
...
<Illustration description="Actors are designed for interoperability. Thanks to the input and output schemas, it easy to connect Actors with external systems, be it directly via REST API, Node.js or Python clients, CLI, or no-code automations." />
...
<Illustration
    description="Actors are notified by the system about various events such as a migration to another server, abort operation triggered by another Actor, or the CPU being overloaded."
    position="content"
/>
...
<Illustration
    description="The Actor can get information about the total and available memory of the Actor’s container or local system. For example, this is useful to auto-scale a pool of workers used for crawling large websites."
    position="content"
/>
...
<Illustration
    description="The Standby mode lets Actors run in the background and respond to incoming HTTP requests, like a web or API server."
    position="left"
/>
...
<Illustration
    description="To run an Actor on the Apify platform or another cloud platform, a user typically needs to pay to cover the computing costs."
    position="right"
/>
