...
import illuAPIAbort from './illu-api-abort.png';
import illuAPIChargingMoney from './illu-api-charging-money.png';
import illuAPIExit from './illu-api-exit.png';
import illuApifyStore from './illu-apify-store.png';
import illuAPIGetInput from './illu-get-input.png';
import illuAPIInitialization from './illu-api-initialization.png';
import illuAPIKeyValueStoreAccess from './illu-api-key-value-store-access.png';
import illuAPIMetamorph from './illu-api-metamorph.png';
import illuAPIPush from './illu-api-push.png';
import illuAPIReboot from './illu-api-reboot.png';
import illuAPIStartAnother from './illu-api-start-another.png';
import illuAPIStatus from './illu-api-status.png';
import illuAPIWebHook from './illu-api-webhook.png';
import illuAPIWebServer from './illu-api-webserver.png';
import illuBasicConceptsInput from './illu-basic-concepts-input.png';
import illuBasicConceptsIntegrations from './illu-basic-concepts-integrations.png';
import illuBasicConceptsOutput from './illu-basic-concepts-output.png';
import illuBasicConceptsRunEnvironment from './illu-basic-concepts-docker.png';
import illuBasicConceptsStorage from './illu-basic-concepts-storage.png';
import illuBasicConceptsStorageDataset from './illu-basic-concepts-storage-dataset.png';
import illuBasicConceptsStorageKeyValueStore from './illu-basic-concepts-storage-key-value-store.png';
import illuDefinitionFiles from './illu-definition-files.png';
import illuDefinitionFilesInputSchemaFile from './illu-definition-files-input-schema-file.png';
import illuDefinitionFilesOutputSchemaFile from './illu-definition-files-output-schema-file.png';
import illuDiagram from './illu-diagram.png';
import illuFutureWork from './illu-future-work.png';
import illuPhilosophyWhyTheName from './illu-philosophy-why-the-name.png';
import Illustration from '../../components/Illustration.astro';
import illuTakerInput from './illu-taker-input.png';
...
This whitepaper introduces **_Actors_**, a new kind of serverless programs for general-purpose language-agnostic computing and automation jobs (also known as agents, functions, apps, ...). The main goal for Actors is to make it easy for developers build and ship reusable software tools, which are also easy to run and integrate by others. For example, Actors are useful for building web scrapers, crawlers, automations, and AI agents.
...
<Picture src={illuDiagram} alt="Actor drawing" formats={['avif', 'webp']} />
...
<Illustration description="Actors" position="content" image={illuPhilosophyWhyTheName} />
...
<Illustration
    description="The SDKs provide convenience methods to initialize the Actor and handle its result. During initialization, the SDK loads environment variables, checks configuration, prepares to receive system events, and optionally purges previous state from local storage."
    position="right"
    image={illuAPIInitialization}
/>
...
<Illustration
    description="Write and read arbitrary files using a storage called [Key-value store](https://sdk.apify.com/docs/api/key-value-store). When an Actor starts, by default it is associated with a newly-created key-value store, which only contains one file with input of the Actor (see [Get input](#get-input))."
    position="content"
...
<Illustration
    description="When the main Actor process exits (i.e. the Docker container stops running), the Actor run is considered finished and the process exit code is used to determine whether the Actor has succeeded or failed."
    position="right"
    image={illuAPIExit}
/>
...
<Illustration
    description="Each Actor run has a status (the `status` field), which indicates its stage in the Actor's lifecycle. The status can be one of the following values:"
    position="right"
    image={illuAPIStatus}
/>
...
<Illustration
    description="Actor can start other Actors, if they have a permission. It can override the default dataset or key-value store, and e.g. forwarding the data to another named dataset, that will be consumed by the other Actor."
    position="content"
...
<Illustration
    description="Run another Actor or an external HTTP API endpoint after Actor run finishes or fails."
    position="right"
    image={illuAPIWebHook}
/>
...
<Illustration
    description="Abort itself or another Actor running on the Apify platform. Aborting an Actor changes its [status](#actor-status) to `ABORTED`."
    position="right"
    image={illuAPIAbort}
/>
...
<Illustration
    description="To run an Actor on the Apify platform or another cloud platform, a user typically needs to pay to cover the computing costs. Additionally, the platforms are free to introduce other [monetization](#monetization) mechanisms, such as charging the users a fixed monthly fee for 'renting' the Actor, or a variable fee for the number of results produced by the Actor. These charging mechanisms are beyond the scope of this whitepaper."
    position="right"
    image={illuAPIChargingMoney}
/>
...
<Illustration
    description="The Actor system uses several special files that define Actor metadata, documentation, instructions how to build and run it, input and output schema, etc. These files MUST be stored in the `.actor` directory placed in Actor's top-level directory."
    position="content"
    image={illuDefinitionFiles}
/>
...
<Illustration
    description="Similarly to input, Actors can generate an [output](#output) JSON object, which links to their results. The Actor output schema file defines how such output object looks like, including types of its properties and description. This file is referenced in the Actor file (`.actor/actor.json`) file as the `output` property. It is a standard JSON Schema file with our extensions, and it is typically stored at `.actor/output_schema.json`."
    position="right"
...
<Illustration description="" position="center" image={illuFutureWork} transparent />
